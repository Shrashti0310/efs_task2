...
resource "tls_private_key" "task2_keypair" {
  algorithm   = "RSA"
...
resource "aws_key_pair" "task2_keypair"{
      ...
      public_key = tls_private_key.task2_keypair.public_key_openssh
}
...
resource "aws_instance" "task2_instance" {
     ...
     key_name = "task2_keypair"
     security_groups = ["${aws_security_group.task2-sg.tags.Name}"]

 connection {
    type     = "ssh"
    user     = "ec2-user"
    private_key = tls_private_key.task2_keypair.private_key_pem
    host     = aws_instance.task2_instance.public_ip
  }
 ...
 depends_on = [ aws_efs_mount_target.alpha]
  ...
  provisioner "remote-exec" {
      inline = [
        ...
        "sudo curl https://raw.githubusercontent.com/Shrashti0310/cloud_task2/master/code.html > code.html",     
...
resource "aws_s3_bucket_object" "task2-object" {
  ...
  key    = "someobject"
  source = "C:/Users/shrashti/Desktop/nature.jpg"
  etag = "C:/Users/shrashti/Desktop/nature.jpg"
  acl = "public-read"
